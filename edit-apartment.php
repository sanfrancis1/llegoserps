...
echo("<script type='text/javascript'> console.log('Test');</script>");
    ...
    if(!empty($apartmentstatus)){
        $query=mysqli_query($con,"UPDATE apartment set apartment_status='$apartmentstatus' where  ID='$eid'");
    }
    if($vehiclecount >= 0){
        $query=mysqli_query($con,"UPDATE apartment set vehicle_count='$vehiclecount' where  ID='$eid'");
    }
   
   // $query=mysqli_query($con,"UPDATE apartment set apartment_status='$apartmentstatus' where  ID='$eid'");
 
    if ($query){
        $msg="Apartment Detail has been updated";
    } else {
              ...
              <div class="row">
				...
				<!-- /.form-group -->
                   <div class="col-md-6">
                    ...
                    <input type="text" class="form-control"value="<?php  echo $row['apartment_status'];?>" disabled >
                  </div>
              </div>
            <div class="col-md-6">
         			   <div class="form-group">
          		   <label>New Company Name</label>
           			 <input type="text" class="form-control" name = 'apartmentstatus'>    
                  </div>
             </div>
             
             <div class="col-md-6">
           			 <div class="form-group">
                    <label>Allocated Vehicle Count</label>
                    <input type="text" class="form-control"value="<?php  echo $row['vehicle_count'];?>" disabled >
                  </div>
              </div>
            <div class="col-md-6">
         			   <div class="form-group">
          		   <label>Change Allocated Vehicle Count</label>
           			 <input type="text" class="form-control" name = 'vehiclecount'>    
                  </div>
             </div>
